package PaperlessOffice::GUI::Tab::View::ScanNewDocuments;
...
use Class::MethodMaker
  ...
  [
   ...
   MyDocumentManager Data ScannerData ImageDir /
...
sub init {
  my ($self,%args) = @_;
  $self->Data({});
  $self->ScannerData($args{ScannerData});
  ...
  $self->Top1
    ($self->MyMainWindow->Toplevel
     (
      ...
      $UNIVERSAL::paperlessoffice->MyGUI->MyConfiguration->PrintScannerName
      (Scanner => $self->ScannerData),
     ));
  # pack all the buttons
  # we want to first
  $self->Pages([]);
  $self->Counter(0);
  $self->ImageDir(ConcatDir('/tmp/paperless-office',$self->GetScannerDir()));
  if (! -d $self->ImageDir) {
    mkdir($self->ImageDir);
  }
}
...
sub GetScannerDir {
  my ($self,%args) = @_;
  my $scannerString = $self->ScannerData->{'scanner-string'};
  if (! defined $scannerString) {
    # throw error
  }
  $scannerString =~ s/(\W)/_/sg;
...
#   my ($self,%args) = @_;
#   # FIXME: have this dependent on which printer is selected
#   $self->ScanNewImage
#     (
#      Mode => "brother4-adf",
#     );
...
sub ActionDone {
  ...
  foreach my $doc (@{$self->Pages}) {
    ...
    my $continue = 1;
    while ($continue) {
      my $tmpfile = ConcatDir($doc->Dir,"$i.pnm");
      if (-f $tmpfile) {
	push @commands, "cp ".shell_quote($tmpfile)." ".shell_quote(ConcatDir($tempdir,$counter.".pnm"));
	++$counter;
	++$i;
      } else {
	$continue = 0;
...
sub ScanNewImage {
  ...
  my $fn = ConcatDir($self->ImageDir,'0.pnm');
  ...
  if ($args{Mode} eq "hp-adf") {
    $self->StartScanFinishedWatcherFn
      (
       Filename => $fn,
       FilenameTemplate => ConcatDir($self->ImageDir,"/hpscan_pg\%s_001.png"),
      );
  } else {
    $self->StartScanFinishedWatcherFn
      (
  ...
  if (exists $UNIVERSAL::paperlessoffice->Conf->{'-f'}) {
    ...
    if ($args{Mode} eq "hp-adf") {
      push @commands, "(cd ".shell_quote($self->ImageDir)." && sudo /var/lib/myfrdcsa/codebases/minor/paperless-office/scripts/hp-scan-paperless-office-2 --adf --device=".shell_quote($self->ScannerData->{'scanner-string'})." --res=$resolution --area=$papersize --mode ".shell_quote(lc($mode))." > ".shell_quote($fn)."; touch ".shell_quote($scanfinishedtimestamp).") &";
    } elsif ($args{Mode} eq "brother4-adf") {
      # FIXME: Add the rest of the params
      push @commands, "(cd ".shell_quote($self->ImageDir)." && scanimage -d ".shell_quote($self->ScannerData->{'scanner-string'})." --source ".shell_quote("Automatic Document Feeder(centrally aligned)")." --batch=\"\%d.pnm\" --res=$resolution $papersize --mode \"24bit Color[Fast]\"; touch ".shell_quote($scanfinishedtimestamp).") &";
    } elsif ($args{Mode} eq "brother4") {
      # FIXME: Add the rest of the params
      push @commands, "(sudo scanimage -d ".shell_quote($self->ScannerData->{'scanner-string'})." --resolution $resolution $papersize --mode $mode > ".shell_quote($fn)."; touch ".shell_quote($scanfinishedtimestamp).") &";
    } else {
      push @commands, "(sudo scanimage -d ".shell_quote($self->ScannerData->{'scanner-string'})." --resolution $resolution $papersize --mode $mode > ".shell_quote($fn)."; touch ".shell_quote($scanfinishedtimestamp).") &";
    }
  }
...
sub StartScanFinishedWatcherFn {
  ...
  $self->ScanFinishedWatcher->{ScanFinishedTimestamp} = ConcatDir($self->ImageDir, "scan-finished.timestamp");
  ...
  my $toplevel = $self->Top1->Toplevel
      (
       -title => 'Scan New Documents: '.$self->ScannerData->{'scanner-string'},
      );
  $toplevel->Label
    (
...
sub ScanFinishedWatcherFn {
  ...
  if (-f $self->ScanFinishedWatcher->{ScanFinishedTimestamp}) {
	...
	if (-f $tmpfn) {
	  system "sudo chmod 755 ".shell_quote($tmpfn);
	  system "convert ".shell_quote($tmpfn)." ".shell_quote(ConcatDir($self->ImageDir, ($i-1).".pnm"));
	  system "sudo rm ".shell_quote($tmpfn);
	  ++$i;
	} else {
	  $continue = 0;
	}
...
sub UpdateWithNewScannedItem {
  ...
  if (-f $args{Filename}) {
    ...
    # FIXME ultimately deal with new scanned images here, instead of
    # inside the document, so that the pages are all displayed also
    # deal with the case where there is no page added
    ...
    # if adf, copy over the files and ensure it is initialized
    $UNIVERSAL::paperlessoffice->MyDocumentManager->AddDocument
      (
       Directory => basename($tempdir),
       DocumentDirectory => $directory,
